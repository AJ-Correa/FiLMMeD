...
class VRPTrainer:
    ...
    def run(self):
        ...
        for epoch in range(self.start_epoch, self.trainer_params['epochs'] + 1):
            ...
            if self.env_params['curriculum'] and epoch == int(
                    self.trainer_params['epochs'] * self.env_params['curriculum_schedule'][0]):
                # self.task_set.extend(["MDOVRPTW",
                #             "MDOVRPB", "MDOVRPL", "MDVRPBL", "MDVRPBTW", "MDVRPLTW", "MDVRPIB",
                #              "MDVRPIL", "MDVRPITW"])
                self.task_set.extend(["MDOVRPB", "MDOVRPL", "MDVRPBL", "MDVRPLTW", "MDVRPIB",
            ...
            self.logger.info("Epoch {:3d}/{:3d}({:.2f}%): Time Est.: Elapsed[{}], Remain[{}]".format(
                epoch, self.trainer_params['epochs'], epoch / self.trainer_params['epochs'] * 100, elapsed_time_str,
                remain_time_str))
    ...
    def _val_and_stat(self, dir, val_path, batch_size=32, val_episodes=1000, task_name=None):
        ...
        if baseline_score is not None:
            ...
            print(
                f">> Val Score on {task_name}: NO_AUG_Score: {no_aug_score_list[0]} --> AUG_Score: {aug_score_list[0]} | Baseline_Gap: {gap:.2f}%")
        else:
            print(
                f">> Val Score on {task_name}: NO_AUG_Score: {no_aug_score_list[0]} --> AUG_Score: {aug_score_list[0]} | Baseline not available")
