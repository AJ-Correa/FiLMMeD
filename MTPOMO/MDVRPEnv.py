...
from VRProblemDef import get_random_problems_mixed, augment_xy_data
...
class MDVRPEnv:
    ...
    def load_problems(self, batch_size, aug_factor=1, problems=None, is_benchmark=False):
        ...
        if problems is None:
            depot_xy, node_xy, node_demand, node_earlyTW, node_lateTW, node_servicetime, is_open, is_interdepot, route_length_limit = get_random_problems_mixed(
                batch_size, self.problem_size, self.depot_size, self.problem_type)
        elif is_benchmark:
            depot_xy = problems[0]
            node_xy = problems[1]
            node_demand = problems[2]
            node_earlyTW = problems[5]
            node_lateTW = problems[6]
            node_servicetime = problems[4]
            self.problem_size = node_xy.size(1)
            self.pomo_size = self.problem_size + self.depot_size - 1
            self.depot_size = depot_xy.size(1)

            # Read is_open and is_interdepot from data tuple if provided (positions 7 and 8)
            if len(problems) > 7:
                is_open = problems[7]
                is_interdepot = problems[8]
            else:
                # Fallback for backward compatibility
                is_open = torch.zeros(size=(batch_size, self.problem_size))
                is_interdepot = torch.zeros(size=(batch_size, self.problem_size))
            route_length_limit = problems[3]

            self.depot_late_tw = 100
        else:
            depot_xy = problems[0]
            node_xy = problems[1]
            node_demand = problems[2]
        ...
        if is_interdepot.sum() > 0:
            self.attribute_id = True
                
            dummy_xy = depot_xy.clone()
            
            dummy_demand = torch.zeros_like(depot_xy[:, :, 0])  # (batch, depot_size)
            
            dummy_servicetime = torch.zeros_like(dummy_demand)
            dummy_earlyTW = torch.zeros_like(dummy_demand)
            dummy_lateTW = torch.ones_like(dummy_demand) * self.depot_late_tw if (node_lateTW.sum() > 0) else torch.zeros_like(dummy_demand)
            
            node_xy = torch.cat([node_xy, dummy_xy], dim=1)
            node_demand = torch.cat([node_demand, dummy_demand], dim=1)
            node_earlyTW = torch.cat([node_earlyTW, dummy_earlyTW], dim=1)
            node_lateTW = torch.cat([node_lateTW, dummy_lateTW], dim=1)
            node_servicetime = torch.cat([node_servicetime, dummy_servicetime], dim=1)
            
            additional_route_flags  = torch.full((is_open.size(0), 3), is_open[0, 0].item())
            is_open = torch.zeros(size=(self.batch_size, self.problem_size + self.depot_size))
            is_interdepot = torch.ones(size=(self.batch_size, self.problem_size + self.depot_size))
            if aug_factor > 1:
                is_open.repeat(aug_factor, 1)
                is_interdepot.repeat(aug_factor, 1)
        else:
            self.attribute_id = False
        ...
        if (node_demand < 0).any():
            ...
            self.pomo_size = min(int(self.problem_size * (1 - 0.2)), self.pomo_size) + self.depot_size - 1
            
            self.start_node = torch.arange(start=1, end=self.problem_size + self.depot_size)[None, :].expand(
                    self.batch_size, -1)
            
            if self.attribute_id:
                self.start_node = self.start_node[self.depot_node_demand[:, 1:-self.depot_size] >= 0].reshape(self.batch_size, -1)[:,
                                  :self.pomo_size]
                is_backhaul = torch.ones(size=(self.batch_size, self.problem_size + self.depot_size))
            else:
            ...
            self.attribute_b = False
            self.pomo_size = self.env_params.get('pomo_size', self.problem_size + self.depot_size - 1)

            self.start_node = torch.arange(start=1, end=self.pomo_size + 1)[None, :].expand(self.batch_size, -1)
            
            is_backhaul = torch.zeros(size=(self.batch_size, self.problem_size + self.depot_size if self.attribute_id else self.problem_size))
            if aug_factor > 1:
                is_backhaul.repeat(aug_factor, 1)
            # shape: (batch, problem)
            self.is_backhaul = is_backhaul

        self.batch_indices = torch.arange(self.batch_size).unsqueeze(1).expand(self.batch_size, self.pomo_size)
        self.pomo_indices = torch.arange(self.pomo_size).unsqueeze(0).expand(self.batch_size, self.pomo_size)
        ...
        if (node_lateTW.sum() > 0):            
            ...
            depot_lateTW = torch.ones(size=(self.batch_size, self.depot_size)) * self.depot_late_tw
            
            is_tw = torch.ones(size=(self.batch_size, self.problem_size + self.depot_size if self.attribute_id else self.problem_size))
            if aug_factor > 1:
            ...
            is_tw = torch.zeros(size=(self.batch_size, self.problem_size + self.depot_size if self.attribute_id else self.problem_size))
            if aug_factor > 1:
                is_tw.repeat(aug_factor, 1)
            # shape: (batch, problem)
            self.is_tw = is_tw
        ...
        num_nodes = self.problem_size + self.depot_size if self.attribute_id else self.problem_size
        # Expand to (batch, pomo, num_nodes)
        self.route_limit = route_limit.view(self.batch_size, 1, 1).expand(-1, self.pomo_size, num_nodes)
        ...
        if (route_length_limit.sum() > 0):
            ...
            is_limit = torch.ones(size=(self.batch_size, self.problem_size + self.depot_size if self.attribute_id else self.problem_size))
            if aug_factor > 1:
            ...
            self.attribute_l = False
            is_limit = torch.zeros(size=(self.batch_size, self.problem_size + self.depot_size if self.attribute_id else self.problem_size))
            if aug_factor > 1:
                is_limit.repeat(aug_factor, 1)
            # shape: (batch, problem)
            self.is_limit = is_limit
    ...
    def reset(self):
        ...
        # shape: (batch, pomo)
        if self.attribute_id:
            self.is_interdepot = torch.ones((self.batch_size, self.pomo_size))
        else:
        ...
        if self.attribute_id:
            ...
            self.ninf_mask = torch.zeros(size=(self.batch_size, self.pomo_size, self.problem_size + (self.depot_size * 2)))
            # shape: (batch, pomo, problem+depot*2)
        else:
            self.visited_ninf_flag = torch.zeros(
        ...
        # shape: (batch, pomo)
        self.used_cap_backhaul = torch.zeros(size=(self.batch_size, self.pomo_size))
        self.used_cap_linehaul = torch.zeros(size=(self.batch_size, self.pomo_size))
    ...
    def step(self, selected):
        ...
        if self.attribute_b:
            self.used_cap_backhaul[selected_demand < 0] -= selected_demand[selected_demand < 0]
            self.used_cap_linehaul[selected_demand > 0] += selected_demand[selected_demand > 0]
            self.used_cap_backhaul[self.at_the_depot] = 0.
            self.used_cap_linehaul[self.at_the_depot] = 0.

        #### mask nodes if load exceed ###

        # shape: (batch, pomo, problem+1)
        self.visited_ninf_flag[:, :, 0:self.depot_size][self.at_the_depot] = 0
        self.visited_ninf_flag[:, :, 0:self.depot_size][self.last_at_the_depot] = float('-inf')

        self.visited_ninf_flag[:, :, 0:self.depot_size][self.last_at_the_depot + ~self.at_the_depot] = float('-inf')
        self.visited_ninf_flag[self.BATCH_IDX, self.POMO_IDX, self.departure_depot] = 0
        self.visited_ninf_flag[self.BATCH_IDX, self.POMO_IDX, selected] = float('-inf')
        
        if self.attribute_id:
            # if solution is not finished and load not at maximum capacity, allow dummy depot visits
            can_refill_dummy_mask = (~self.finished) & (self.load < 1)
            self.load[selected >= (self.problem_size + self.depot_size)] = 1
            self.visited_ninf_flag[self.BATCH_IDX[can_refill_dummy_mask], self.POMO_IDX[can_refill_dummy_mask], self.problem_size + self.depot_size:] = 0
            #self.visited_ninf_flag[self.BATCH_IDX, self.POMO_IDX, self.departure_depot + self.problem_size + self.depot_size] = float('-inf')
            
            # if solution is finished or load is already at maximum, block dummy depot visits
            block_dummy_mask = (self.finished | (self.load == 1))
            self.visited_ninf_flag[self.BATCH_IDX[block_dummy_mask], self.POMO_IDX[block_dummy_mask], self.problem_size + self.depot_size:] = float('-inf')
        ...
        if self.attribute_b:
            ...
            self.used_cap_backhaul[reset_index] = 0.
            self.used_cap_linehaul[reset_index] = 0.

            self.last_at_the_depot = self.at_the_depot
            self.ninf_mask = self.visited_ninf_flag.clone()
            round_error_epsilon = 0.00001

            exceeds_cap_linehaul = demand_list + self.used_cap_linehaul[:, :, None] > 1.0 + round_error_epsilon
            exceeds_cap_backhaul = -demand_list + self.used_cap_backhaul[:, :, None] > 1.0 + round_error_epsilon
            cannot_serve_linehaul = demand_list > 1.0 + round_error_epsilon - self.used_cap_backhaul[:, :, None]
            self.ninf_mask[exceeds_cap_linehaul] = float('-inf')
            self.ninf_mask[exceeds_cap_backhaul] = float('-inf')
            self.ninf_mask[cannot_serve_linehaul] = float('-inf')
        ...
        if (self.attribute_tw):
            ...
            fail_return_depot = arrival_time + self.depot_node_servicetime[:, None, :].expand(-1, self.pomo_size,
                                                                                              -1) + distance_tensor > self.depot_late_tw + round_error_epsilon
        ...
        if (self.attribute_l):
            # shape: (batch, pomo)
            depot_xy = xy_list[self.batch_indices, self.pomo_indices, self.departure_depot, :]
        ...
        if self.attribute_id:
            newly_finished = (self.visited_ninf_flag[:, :, self.depot_size:self.problem_size + self.depot_size] == float('-inf')).all(dim=2)
        ...
        self.ninf_mask[self.batch_indices[self.finished], self.pomo_indices[self.finished], self.last_depot[
    ...
    def load_dataset(self, path, offset=0, num_samples=1000, device='cuda'):

        assert os.path.splitext(path)[1] == ".pkl", "Unsupported file type (.pkl needed)."
        with open(path, 'rb') as f:
            data = pickle.load(f)

        depot_xy = torch.from_numpy(data['depot_xy'][offset: offset + num_samples]).to(device)
        node_xy = torch.from_numpy(data['node_xy'][offset: offset + num_samples]).to(device)
        node_demand = torch.from_numpy(data['node_demand'][offset: offset + num_samples]).to(device)
        node_earlyTW = torch.from_numpy(data['node_earlyTW'][offset: offset + num_samples]).to(device)
        node_lateTW = torch.from_numpy(data['node_lateTW'][offset: offset + num_samples]).to(device)
        node_serviceTime = torch.from_numpy(data['node_serviceTime'][offset: offset + num_samples]).to(device)
        is_open = torch.from_numpy(data['route_open'][offset: offset + num_samples]).to(device)
        is_interdepot = torch.from_numpy(data['route_interdepot'][offset: offset + num_samples]).to(device)
        route_length_limit = torch.from_numpy(data['route_length_limit'][offset: offset + num_samples]).to(device)
