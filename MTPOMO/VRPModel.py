...
class VRPModel(nn.Module):
    ...
    def pre_forward(self, reset_state):
        ...
        self.encoded_nodes = self.encoder(depot_xy, node_xy_demand_TW, attribute_vector)

        # shape: (batch, problem+1, embedding)
        self.decoder.set_kv(self.encoded_nodes)

    def forward(self, state, route=None, return_probs=False):
        ...
        if state.selected_count == 0:  # First Move, depot
            ...
            probs = self.decoder(encoded_last_node, state.load, state.time, state.length, state.is_open, state.is_interdepot, ninf_mask=state.ninf_mask)
...
class VRP_Decoder(nn.Module):
    ...
    def forward(self, encoded_last_node, load, time, length, route_open, route_interdepot, ninf_mask):
        ...
        #######################################################
        input_cat = torch.cat((encoded_last_node, load[:, :, None], time[:, :, None],length[:, :, None], route_open[:, :, None], route_interdepot[:, :, None]), dim=2)
