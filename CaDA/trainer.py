...
class VRPTrainer:
    ...
    def run(self):
        ...
        for epoch in range(self.start_epoch, args.trainer_params["epochs"] + 1):
            ...
            with Progress(
                ...
                while instances_seen < train_episodes:
                    ...
                    with torch.amp.autocast(
                        ...
                        reld_alpha = step / total_steps if epoch == 1 else 1.0
                        out = self.model(
                            td, self.env, reld_alpha=reld_alpha
                        )
                    ...
                    progress.update(
                        train_task,
                        description=f"{train_label} | {metric_info}",
                        advance=batch_size,
                    )
            ...
            args.log(
                f"{train_label} | {elapsed} | {metric_info} | LR {self.optimizer.param_groups[0]['lr']:.2e}"
            )
            ## on all devices
            if args.ddp:
