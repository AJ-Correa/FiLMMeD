...
class MTVRPEnv(EnvBase):
    ...
    def get_action_mask(td: TensorDict) -> torch.Tensor:
        ...
        exceeds_cap_linehaul = (
            td["demand_linehaul"] + td["used_capacity_linehaul"] > td["vehicle_capacity"] + EPS
        )
        exceeds_cap_backhaul = (
            td["demand_backhaul"] + td["used_capacity_backhaul"] > td["vehicle_capacity"] + EPS
        )
    ...
    def select_start_nodes(self, td, po_B: Optional[int] = None):
        """Select start nodes for multi-start decoding (POMO-style).

        When `self.loss_mode == 'po'` and `po_B` is provided, this function will produce
        exactly `B=po_B` start nodes (per instance) following the user's specification:

        - The first start is always node 0 (depot) and will be marked as the greedy trajectory.
        - Next include the standard POMO starts: nodes 1..customer_nodes.
        - If B > (1 + customer_nodes), repeat the full POMO starts as many times as needed
          (these are duplicates of 1..customer_nodes), and for any remaining slots sample
          unique customer nodes (without replacement among themselves) to fill to B.
        - If B <= (1 + customer_nodes), take the first B starts from the list above
          (keeping node 0 as the first entry if B >= 1).

        Returns:
            num_starts: int, number of starts per instance (B or default)
            selected: Tensor of shape (num_starts * batch,) with start node indices
            greedy_mask: BoolTensor of same shape, True only for entries that correspond to depot start (0)
        """
        batch = td.batch_size[0]
        device = td["locs"].device
        customer_nodes = td["locs"].shape[-2] - 1
        # base POMO starts (1..customer_nodes)
        pomo_starts = torch.arange(1, customer_nodes + 1, dtype=torch.long, device=device)
        if getattr(self, "loss_mode", "rl") == "po" and po_B is not None:
            B = int(po_B)
            # Ensure at least 1 (depot)
            if B < 1:
                B = 1
            base_list = [0] + [int(x) for x in pomo_starts.tolist()]
            base_len = len(base_list)
            if B <= base_len:
                # take first B starts (keep depot as first if B>=1)
                chosen = base_list[:B]
            else:
                # need to extend
                ...
                # add full replications of 1..customer_nodes
                for _ in range(full_replications):
                    chosen.extend([int(x) for x in pomo_starts.tolist()])
                # add remainder as a random non-repeating selection from 1..customer_nodes
                if remainder > 0:
            ...
            # default behavior (POMO for rl or no po_B provided)
