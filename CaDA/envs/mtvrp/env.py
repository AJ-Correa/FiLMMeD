...
def get_dataloader(dataset, batch_size, ddp=False, num_workers=0):
    ...
    if isinstance(dataset, dict):
        size_bs = {50: 100, 100: 250, 200: 125, 300: 100}
        batch_size = [
...
class MTVRPEnv(EnvBase):
    ...
    def _reset(
        ...
        if not lib_data:
            # Use same pattern as lib_data: td.get() with tensor-like defaults
            demand_linehaul = td["demand_linehaul"]
            demand_backhaul = td["demand_backhaul"]
            
            locs = td["locs"]

            if demand_linehaul.shape[1] < locs.shape[1]:
                demand_linehaul = torch.cat([torch.zeros(size=(*batch_size, 1)), td["demand_linehaul"]], dim=1)
            if demand_backhaul.shape[1] < locs.shape[1]:
                demand_backhaul = torch.cat([torch.zeros(size=(*batch_size, 1)), td["demand_backhaul"]], dim=1)

            # Handle missing constraint fields with tensor-like defaults (mirrors lib_data)
            backhaul_class = td.get("backhaul_class", torch.ones((*batch_size, 1), dtype=torch.float32, device=device))
            distance_limit = td.get("distance_limit", torch.full_like(demand_linehaul[..., :1], float("inf")))
            service_time = td.get("service_time", torch.zeros_like(demand_linehaul))
            
            time_windows = td.get("time_windows", None)
            if time_windows is None:
                time_windows = torch.zeros_like(locs)
                time_windows[..., 1] = float("inf")
            
            open_route = td.get("open_route", torch.zeros_like(demand_linehaul[..., :1], dtype=torch.bool))
            vehicle_capacity = td.get("vehicle_capacity", torch.ones_like(demand_linehaul[..., :1]))
            capacity_original = td.get("capacity_original", torch.ones_like(demand_linehaul[..., :1]))
