...
class MOEModel(nn.Module):
    ...
    def pre_forward(self, reset_state):
        ...
        # shape: (batch, problem)
        node_serviceTime = reset_state.node_serviceTime
        # shape: (batch, problem)
        node_xy_demand = torch.cat((node_xy, node_demand[:, :, None]), dim=2)
        # shape: (batch, problem, 3)
        node_TW = torch.cat((node_earlyTW[:, :, None], node_lateTW[:, :, None], node_serviceTime[:, :, None]), dim=2)
        # shape: (batch, problem, 2)
        node_xy_demand_TW = torch.cat((node_xy_demand, node_TW), dim=2)
        # shape: (batch, problem, 6)
        attribute_vector = torch.cat((reset_state.is_open[:, :, None], reset_state.is_tw[:, :, None],
                                      reset_state.is_backhaul[:, :, None], reset_state.is_limit[:, :, None],
                                      reset_state.is_interdepot[:, :, None]), dim=2)

        self.encoded_nodes, moe_loss = self.encoder(depot_xy, node_xy_demand_TW, attribute_vector)
    ...
    def forward(self, state, selected=None, return_probs=False):
        ...
        if state.selected_count == 0:  # First Move, depot
            ...
            probs, moe_loss = self.decoder(encoded_last_node, state.load, state.time, state.length, state.is_open,
                                           state.is_interdepot, ninf_mask=state.ninf_mask)
            self.aux_loss += moe_loss
            ...
            if selected is None:
                while True:
                    if self.training or self.eval_type == 'softmax':
                        try:
                            selected = probs.reshape(batch_size * pomo_size, -1).multinomial(1).squeeze(dim=1).reshape(
...
class MTL_Encoder(nn.Module):
    def __init__(self, **model_params):
        ...
        if self.model_params['num_experts'] > 1 and "Raw" in self.model_params['expert_loc']:
            ...
            self.embedding_node = MoE(input_size=6, output_size=embedding_dim,
                                      num_experts=self.model_params['num_experts'],
                                      k=self.model_params['topk'], T=1.0, noisy_gating=True,
                                      routing_level=self.model_params['routing_level'],
                                      routing_method=self.model_params['routing_method'], moe_model="Linear")
    ...
    def forward(self, depot_xy, node_xy_demand_tw, attribute_vector):
        ...
        if self.model_params['use_film']:
            modulated_node = self.film(embedded_node, attribute_vector)
            out = torch.cat((embedded_depot, modulated_node), dim=1)
        else:
...
class EncoderLayer(nn.Module):
    def __init__(self, depth=0, **model_params):
        ...
        if self.model_params['num_experts'] > 1 and "Enc{}".format(depth) in self.model_params['expert_loc']:
            ...
            self.feedForward = MoE(input_size=embedding_dim, output_size=embedding_dim,
                                   num_experts=self.model_params['num_experts'],
                                   hidden_size=self.model_params['ff_hidden_dim'], k=self.model_params['topk'], T=1.0,
                                   noisy_gating=True,
                                   routing_level=self.model_params['routing_level'],
                                   routing_method=self.model_params['routing_method'], moe_model="MLP")
        else:
    ...
    def forward(self, input1):
        ...
        if self.model_params['norm_loc'] == "norm_last":
            ...
            # Convert input to float32 for MOE
...
class MTL_Decoder(nn.Module):
    def __init__(self, **model_params):
        ...
        if self.model_params['num_experts'] > 1 and 'Dec' in self.model_params['expert_loc']:
            self.multi_head_combine = MoE(input_size=head_num * qkv_dim, output_size=embedding_dim,
                                          num_experts=self.model_params['num_experts'],
                                          k=self.model_params['topk'], T=1.0, noisy_gating=True,
                                          routing_level=self.model_params['routing_level'],
                                          routing_method=self.model_params['routing_method'], moe_model="Linear")
    ...
    def forward(self, encoded_last_node, load, time, length, route_open, route_interdepot, ninf_mask):
        ...
        #######################################################
        input_cat = torch.cat(
            (encoded_last_node, load[:, :, None], time[:, :, None], length[:, :, None], route_open[:, :, None],
             route_interdepot[:, :, None]), dim=2)
...
class FiLM(nn.Module):
    ...
    def forward(self, x, cond):
        # x, cond shape: (batch, problem, feature_dim)
