...
class MDVRPEnv:
    ...
    def load_problems(self, batch_size, aug_factor=1, problems=None, is_benchmark=False):
        ...
        if is_interdepot.sum() > 0:
            ...
            dummy_lateTW = torch.ones_like(dummy_demand) * self.depot_late_tw if (
                        node_lateTW.sum() > 0) else torch.zeros_like(dummy_demand)
        ...
        if (node_demand < 0).any():
            ...
            if self.attribute_id:
                self.start_node = \
                self.start_node[self.depot_node_demand[:, 1:-self.depot_size] >= 0].reshape(self.batch_size, -1)[:,
                :self.pomo_size]
            ...
            is_backhaul = torch.zeros(
                size=(self.batch_size, self.problem_size + self.depot_size if self.attribute_id else self.problem_size))
            if aug_factor > 1:
                is_backhaul.repeat(aug_factor, 1)
            # shape: (batch, problem)
            self.is_backhaul = is_backhaul
    ...
    def step(self, selected):
        ...
        if self.attribute_id:
            ...
            self.visited_ninf_flag[self.BATCH_IDX[can_refill_dummy_mask], self.POMO_IDX[
                can_refill_dummy_mask], self.problem_size + self.depot_size:] = 0
            ...
            self.visited_ninf_flag[self.BATCH_IDX[block_dummy_mask], self.POMO_IDX[
                block_dummy_mask], self.problem_size + self.depot_size:] = float('-inf')
        ...
        if self.attribute_id:
            newly_finished = (
                        self.visited_ninf_flag[:, :, self.depot_size:self.problem_size + self.depot_size] == float(
                    '-inf')).all(dim=2)
